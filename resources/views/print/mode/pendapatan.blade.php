...
<table class="ledger">
        ...
        @foreach ($results as $res)
            ...
            <tr class="lvl-0">
                ...
                @foreach ($apbd_perubahan as $ap)
                    ...
                    <td class="num">
                        <span
                            class="varian is-up">{{ number_format($res['total'] - $res['perubahan_totals'][$ap->id]) }}</span>
                    </td>
                @endforeach
            </tr>
            ...
            @foreach ($res['sub_utamas'] as $su)
                ...
                <tr class="lvl-1">
                    ...
                    @foreach ($apbd_perubahan as $ap)
                        ...
                        <td class="num">
                            <span
                                class="varian is-up">{{ number_format($su['total'] - $su['perubahan_totals'][$ap->id]) }}</span>
                        </td>
                    @endforeach
                ...
                @foreach ($su['sub_sutamas'] as $ssu)
                    ...
                    <tr class="lvl-2">
                        ...
                        @foreach ($apbd_perubahan as $ap)
                            ...
                            <td class="num">
                                <span
                                    class="varian is-up">{{ number_format($ssu['total'] - $ssu['perubahan_totals'][$ap->id]) }}</span>
                            </td>
                        @endforeach
                    ...
                    @foreach ($ssu['sub_ssutamas'] as $sssu)
                        ...
                        <tr class="lvl-3">
                            ...
                            @foreach ($apbd_perubahan as $ap)
                                ...
                                <td class="num">
                                    <span
                                        class="varian is-up">{{ number_format($sssu['total'] - $sssu['perubahan_totals'][$ap->id]) }}</span>
                                </td>
                            @endforeach
                        ...
                        @foreach ($sssu['datas'] as $data)
                            ...
                            <tr class="lvl-4">
                                ...
                                <td style="padding-left: 76px"
                                    ...
                                    <span
                                        class=" border border-red-200 text-xs px-2 bg-amber-50">{{ $data->sumberDana->kode }}</span>
                                </td>
                                <td class="num"><span
                                        class="varian">{{ number_format($data->jumlah) }}</span></td>
                                @foreach ($apbd_perubahan as $ap)
                                    @php
                                    ...
                                    <td class="num">
                                        <span
                                            class="varian {{ $total < 0 ? 'is-down' : 'is-up' }}">{{ number_format($total) }}</span>
                                    </td>
                                @endforeach
    ...
    <tfoot>
        <tr>
            ...
            @foreach ($apbd_perubahan as $ap)
                <td class="num">{{ isset($res) ? number_format($res['perubahan_totals'][$ap->id]) : 0 }}
                </td>
                <td class="num">
                    {{ isset($res) ? number_format($totalan - $res['perubahan_totals'][$ap->id]) : number_format($totalan) }}
                </td>
            @endforeach
        </tr>
    </tfoot>
